...
def test_if_statement():
    ...
    assert tree_walker.code == "{% if not foo or foo.bar or foo|bar(foo['hello']) %}\nfoo\n{% endif %}"
...
def test_if_statement2():
    ...
    assert tree_walker.code == "{% if foo %}\nbar\n{% else %}\nfoo{% endif %}"
...
def test_if_statement3():
    ...
    assert tree_walker.code == "{% if foo and awesome.string|banana(\"foo\\\" %s\"|format(a)) %}\nbar\n{% elseif awesome.sauce[1] and blue and \'hello\' %}\nfoo{% endif %}"
...
def test_if_statement4():
    ...
    assert tree_walker.code == "{% if foo|bar(3) or not foo[3] %}\nbar\n{% elseif awesome.sauce[1] and blue and 'hello' %}\nfoo\n{% else %}bar{% endif %}"
...
def test_if_statement5():
    ...
    assert tree_walker.code == "{% if (foo and bar) or foo and (bar or (foo and bar)) %}\nbar\n{% else %}\nfoo{% endif %}"
...
def test_function_statement():
    ...
    assert tree_walker.code == "{{['arg1': bar, 'arg2': 3]|foo}}"
...
def test_function_statement2():
    ...
    assert tree_walker.code == "{{['arg1': bar[1], 'arg2': foo.bar.foo, 'arg3': foo.bar[3], 'arg4': foo.bar.awesome[3]]|foo}}"
...
def test_function_statement3():
    ...
    assert tree_walker.code == "{{[\'arg1\': bar[1]|modifier, \'arg2\': foo.bar.foo, \'arg3\': foo.bar[3]|modifier(array[0], \"hello %s \"|format(foo)), \'arg4\': foo.bar.awesome[3]|modifier2(7, \'hello\', \"hello\", \"%s\"|format(apple.banana))]|foo}}"
...
def test_for_statement():
    ...
    assert tree_walker.code == "{% for bar in foo %}{% endfor %}"
...
def test_for_statement2():
    ...
    assert tree_walker.code == "{% for bar in foo[5].bar[2]|hello(\"world\", \" %s \"|format(hey)) %}bar{% endfor %}"
...
def test_for_statement3():
    ...
    assert tree_walker.code == "{% for bar in foo.bar[2]|hello(\"world\", \" %s \"|format(hey)) %}bar{% else %}{% if not foo %}bar{% endif %}hello{% endfor %}"
