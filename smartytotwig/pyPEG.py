...
def skip(skipper, text, pattern, skipWS, skipComments):
    ...
    if skipComments:
        try:
            while True:
                skip, t = skipper.parseLine(
                    t, skipComments, [], skipWS, None, 0)
                if skipWS:
                    t = t.strip()
        except:
            pass
    ...
    def parseLine(self, textline, pattern, resultSoFar=[], skipWS=True,
        ...
        if pattern_type is type(""):
            if text[:len(pattern)] == pattern:
                text = skip(
                    self.skipper, text[len(pattern):], pattern, skipWS, skipComments)
                return R(None, text)
            else:
                raise SyntaxError()
            ...
            try:
                r, t = self.parseLine(
                    text, pattern.obj, [], skipWS, skipComments, inputLength)
            except:
            ...
            if m:
                text = skip(
                    self.skipper, text[len(m.group(0)):], pattern, skipWS, skipComments)
                if pattern_type is type(ignoring):
                    return R(None, text)
                else:
                    ...
                    if n > 0:
                        for i in range(n):
                            result, text = self.parseLine(
                                text, p, result, skipWS, skipComments, inputLength)
                    elif n == 0:
                        if text == "":
                            pass
                        else:
...
def parseLine(textline, pattern, resultSoFar=[], skipWS=True, skipComments=None, outputPos=False):
    ...
    ast, text = p.parseLine(
        text, pattern, resultSoFar, skipWS, skipComments, length)
    return ast, text
...
def parse(language, lineSource, skipWS=True, skipComments=None, outputPos=False):
    ...
    try:
        ...
        result, text = p.parseLine(
            text, language, [], skipWS, skipComments, length)
        if text:
            raise SyntaxError()
        textlen = 0
    except SyntaxError:
        ...
        print "syntax error in " + lineSource.filename() + ":" + str(lineNo) + ": " + lineCont
        raise
        raise SyntaxError(
            "syntax error in " + lineSource.filename() + ":" + str(lineNo) + ": " + lineCont)
