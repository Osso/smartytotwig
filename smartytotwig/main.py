...
def main() -> None:
    ...
    opt2 = optparse.make_option(
        "-t", "--twig-file", action="store", dest="target", help="Location of the Twig output file."
    )

    opt3 = optparse.make_option(
        "-p", "--twig-path", action="store", dest="path", help="The path used in Twig include tags."
    )
    ...
    parser = optparse.OptionParser(
        usage="smartytotwig --smarty-file=<SOURCE TEMPLATE> --twig-file=<OUTPUT TEMPLATE>"
    )
    ...
    if options.source:
        ...
        with open(options.target, "w", encoding="utf-8") as f:
            f.write(out)
